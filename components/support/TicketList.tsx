...
export default function TicketList({ 
  ...
  // Determine if user can actually view ticket details
  const canViewTicketDetails = (ticket: SupportTicketDetails) => {
    if (userInfo.role === Roles.ADMIN) return true;
    if (userInfo.role === Roles.SUPPORT) {
      return ticket.supportUsers.some(su => su.user.id === userInfo.id);
    }
    // Regular users can view but not edit their own tickets
    return true;
  };
    ...
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6">
        {filteredTickets.map((ticket) => (
          <TicketCard
            key={ticket.id}
            ticket={ticket}
            onClick={canViewTicketDetails(ticket) ? setSelectedTicket : undefined}
      ...
      {selectedTicket && userInfo && (
        <TicketDetailsModal
          ...
          onTicketDeleted={onRefreshNeeded}
        />
      )}
    </>
  );
